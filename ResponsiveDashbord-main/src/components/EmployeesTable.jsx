...
const EmployeesTable = () => {
  ...
  async function getEmployee() {
    let result = await axios.get("http://localhost:4000/api/getAllEmployees");
  ...
  async function deleteEmployee(_id) {
    ...
    if(result == true){
      await axios.delete(`http://localhost:4000/api/deleteEmployees/${_id}`)
      getEmployee()
    }
  }
    ...
    <section>
      <div className='bg-lightsuccess relative rounded-md overflow-x-scroll   '>
        <table className=' text-sm w-full  table-auto text-left text-gray-500'>
          ...
          <tbody className=' '>
            {Array.isArray(data) && currentProducts.map((data) => (
              <tr className='bg-white border-b'>
                ...
                <td className='inline-flex px-6 gap-2 py-4'><Link to={`/admin/employees/viewemployees/${data._id}`} className='text-lightsuccess pointer'><GrFormView size={20} /></Link> 
                  <Link to={`/admin/employees/editemployees/${data._id}`}><FaRegEdit className='text-green-400 pointer' size={20} /></Link>
                  <Link onClick={()=>deleteEmployee(data._id)}><MdOutlineDeleteOutline className='text-red-400 pointer' size={20} /></Link>
                </td>
              </tr>
            ))}
